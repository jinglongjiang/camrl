...
def main():
    ...
    capacity           = train_config.getint('train', 'capacity', fallback=100000)  # 回到 100k
    ...
    if il_episodes > 0:
        ...
        # —— 轻量 BC Sanity Check（不加噪，快速验证口径一致性） ——
        with _quiet_logs():
    ...
    for episode in tqdm(range(train_episodes), desc="Training progress", unit="ep", dynamic_ncols=True):
        ...
        # ---- 目标熵日程：-1.4 → -2.0，覆盖前 70% ----
        ent_t = episode / max(1, int(0.7 * train_episodes))
        target_H_hi, target_H_lo = -1.4, -2.0
        ...
        current_lr_critic = lr_critic * lr_mult   # 已在上方 1.2x 设为基准
        current_lr_alpha  = lr_alpha  * lr_mult   # 已在上方 0.8x 设为基准
        ...
        # —— 温和 BC 日程：p_demo 0.40→0.15；λ_bc 0.90→0.30→0.10 ——
        p_demo    = float(np.interp(t, [0.0, 1.0],        [0.40, 0.15]))
        lambda_bc = float(np.interp(t, [0.0, 0.6, 1.0],   [0.90, 0.30, 0.10]))
        ...
        if len(rl_buf) < 10_000:
            ...
            # 简单自适应：近 100 集成功率 > 0.6 降低少量 SGD 步以稳住策略
