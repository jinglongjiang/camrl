# train.py — IL仅进度条+汇总；IL由专家开车；IL后清空RL回放池；其余保持不变
import sys
import logging
import argparse
import configparser
...
def main():
    ...
    if il_episodes > 0:
        # 临时换成专家（仅为显示/一致性），但 collect_with_labels 里我们直接让“RL=专家、Expert=专家”，
        # 这样由专家动作推进环境，同时也把专家标签写入 exp_buf。
        robot.set_policy(expert_policy)

        succ = coll = tout = 0
        succ_times, ep_rewards = [], []

        bar = tqdm(range(il_episodes), desc="IL (expert drives, collect labels)", dynamic_ncols=True, unit="ep")
        for i in bar:
            # 关键：RL policy 和 Expert policy 都传专家 => 由专家动作推进环境
        ...
        logging.info("IL   done | succ=%.2f coll=%.2f timeout=%.2f nav=%.2f reward=%.4f",
    ...
    for episode in tqdm(range(train_episodes), desc="Training progress", unit="ep", dynamic_ncols=True):
        ...
        if np.random.rand() < p_expert:
            orig = robot.policy
            robot.set_policy(expert_policy)
            explorer.run_k_episodes(1, 'train', update_memory=True, episode=episode, return_stats=False, show_tqdm=True)
        ...
        tqdm.write(f"Episode: {episode + 1}/{train_episodes}, epsilon: {epsilon:.4f}")
