...
# The packed-row columns that depend on the ROBOT rather than on the human or
# its belief. Declared HERE, beside the other packed-row offsets, because a
# consumer that restates them has no way to notice when the row layout moves:
# a silent off-by-one in these five would rewrite the wrong columns of every
# candidate successor and still produce plausible-looking numbers.
# _intent_human_feature_vector builds the scalar block in this order:
#   0 dx | 1 dy | 2 rel_vx | 3 rel_vy | 4 radius | 5 speed | 6 ttc | 7 age ...
# and dx/dy/rel_vx/rel_vy/ttc are the only ones computed against the robot.
HUMAN_SCALAR_IDX_ROBOT_RELATIVE = (0, 1, 2, 3, 6)
...
# (Retired as a symbol by Order 14C; the failed-demos-MC-only rule itself is
# unchanged and is now carried by the V10 contract below.)

# Order 14 section C: ONE quantile grid for every use of the ACTION ranking,
# and it is the deployed one.
#
# Real bug this replaces, measured on the seed-98204 IL-end weights over the
# frozen 768-row audit set. The ranking loss, the warm-up gate and the audit
# all scored candidate actions at the 16 fixed midpoints (2k+1)/32, while
# score_candidates_v5 -- the only scorer that ever drives the robot -- used
# the 32 fixed midpoints (2j+1)/64. Those two point sets are DISJOINT: no
# tau is shared. So action ordering was supervised at 16 taus, verified at
# the SAME 16 taus, and deployed at 32 taus where nothing constrained it.
# The gate could not detect this because it asked exactly the question the
# loss optimised, at exactly the same points.
#
# Measured cost of the split, same weights, same rows, ONLY n_taus differing:
#     loose top-1        88.1%  ->  28.4%
#     speed-ring exact   78.8%  ->  45.4%   (69% of predictions collapse to
#                                            the fastest ring vs 40% expert)
#     argmax changed on 678 of 768 rows; max |score| difference 1.068, larger
#     than the entire 0.623 score range.
#
# MC value regression is NOT affected and keeps sampling tau RANDOMLY
# (iqn_mc_quantiles): it regresses the executed action only, so it never
# defines an ordering over the other 79 and has no deployed counterpart to
# agree with.
ACTION_QUANTILES = 32

TRAINING_CONTRACT_V10_UNIFIED_ACTION_QUANTILES = (
    "bdvl_intent_training_contract_unified_action_quantiles_v10")
...
FROZEN_VALUES: Dict[str, object] = {
    ...
    "iqn_cosines": 64,
    "iqn_mc_quantiles": 16,
    "iqn_action_quantiles": ACTION_QUANTILES,
