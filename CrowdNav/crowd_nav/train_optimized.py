...
class AdaptiveExploration:
    """自适应探索策略"""
    def __init__(self, start_eps=0.3, end_eps=0.05, decay_episodes=2000):
    ...
    def get_epsilon(self, episode, success_rate=None):
        # 基础epsilon衰减
        if episode < self.decay_episodes:
            eps = self.start_eps + (self.end_eps - self.start_eps) * (episode / self.decay_episodes)
        else:
            eps = self.end_eps
            
        # 根据成功率自适应调整
        if success_rate is not None and len(self.success_history) > 10:
            recent_success = np.mean(self.success_history[-10:])
            if recent_success < 0.2:  # 成功率太低，增加探索
                eps = min(eps * 1.5, 0.5)
            elif recent_success > 0.8:  # 成功率太高，减少探索
                eps = max(eps * 0.8, self.end_eps)
                
        return eps
    
    def update_history(self, success_rate, collision_rate):
...
# ----------------- BC预训练 -----------------
def bc_pretrain_actor(policy, exp_buf, device, iters=3000, batch_size=2048, lr=1e-4):
    if len(exp_buf) == 0:
        return 0.0
    actor = policy.actor
    actor.train()
    ...
    for _ in bar:
        ...
        opt.zero_grad(); loss.backward(); torch.nn.utils.clip_grad_norm_(actor.parameters(), 0.5); opt.step()
...
def main():
    ...
    adaptive_exploration = AdaptiveExploration(
        start_eps=train_config.getfloat('train', 'epsilon_start'),
        end_eps=train_config.getfloat('train', 'epsilon_end'),
    ...
    if il_episodes > 0:
        ...
        # BC预训练
        if il_epochs > 0:
            bc_loss = bc_pretrain_actor(policy, exp_buf, device, iters=il_epochs,
    ...
    for episode in tqdm(range(train_episodes), desc="Training progress", unit="ep", dynamic_ncols=True):
        # 自适应探索
        recent_success = np.mean(train_succ_hist[-10:]) if len(train_succ_hist) >= 10 else 0.0
        epsilon = adaptive_exploration.get_epsilon(episode, recent_success)
        ...
        if episode % evaluation_interval == 0:
            ...
            # 早停检查
            if early_stopping(val_stats["success_rate"]):
                logging.info("Early stopping triggered at episode %d", episode)
                break
        ...
        succ_rate = succ/max(1,sample_episodes)
        coll_rate = coll/max(1,sample_episodes)
        adaptive_exploration.update_history(succ_rate, coll_rate)
        
        logging.info("TRAIN(interact) | succ=%.2f coll=%.2f timeout=%.2f reward=%.2f | rl_buf=%d eps=%.3f",
                     succ_rate, coll_rate, tout/max(1,sample_episodes), 
                     (np.mean(ep_rewards) if ep_rewards else 0.0), len(rl_buf), epsilon)
        ...
        meter = trainer.optimize_batch(
            rl_buf, exp_buf, updates=train_config.getint('train', 'train_batches'),
        ...
        if episode % plot_interval == 0 and episode > 0:
            ...
            plt.subplot(221); plt.plot(train_reward_hist, label='Train Avg Reward', alpha=0.7)
            if len(reward_ma) > 0: 
                plt.plot(range(W-1, W-1+len(reward_ma)), reward_ma, label=f'MA({W})', linewidth=2)
            plt.xlabel('Episode'); plt.ylabel('Reward'); plt.legend()
            
            plt.subplot(222); plt.plot(train_succ_hist, label='Success Rate', alpha=0.7)
            if len(succ_ma) > 0: 
                plt.plot(range(W-1, W-1+len(succ_ma)), succ_ma, label=f'MA({W})', linewidth=2)
            plt.xlabel('Episode'); plt.ylabel('Success Rate'); plt.legend()
            
            plt.subplot(223); plt.plot(train_coll_hist, label='Collision Rate', alpha=0.7)
            if len(coll_ma) > 0: 
                plt.plot(range(W-1, W-1+len(coll_ma)), coll_ma, label=f'MA({W})', linewidth=2)
            plt.xlabel('Episode'); plt.ylabel('Collision Rate'); plt.legend()
            
            plt.subplot(224); plt.plot(train_timeout_hist, label='Timeout Rate', alpha=0.7)
            if len(tout_ma) > 0: 
                plt.plot(range(W-1, W-1+len(tout_ma)), tout_ma, label=f'MA({W})', linewidth=2)
            plt.xlabel('Episode'); plt.ylabel('Timeout Rate'); plt.legend()
            
            plt.tight_layout()
            fig_name = os.path.join(args.output_dir, f"train_curves_ep{episode}.png")
            plt.savefig(fig_name, dpi=150); plt.close()
