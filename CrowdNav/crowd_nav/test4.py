...
def main():
    ...
    parser.add_argument('--policy', type=str, default='cadrl', help='Policy name (only cadrl supported)')
    parser.add_argument('--model_dir', type=str, default='runs/mamba_vl', help='Model directory')
    parser.add_argument('--weights', type=str, default='rl_model-cadrl.pth', help='Checkpoint name or "latest"')
    ...
    print("[INFO] ========================================")
    print("[INFO] TEST4.PY - CADRL Baseline Testing")
    print("[INFO] ========================================")
    ...
    for case_idx, case in enumerate(test_cases):
        ...
        # [FIX CADRL] 强制使用checkpoint的架构 (64→32→1)
        if not policy_config.has_section('cadrl'): policy_config.add_section('cadrl')
        policy_config.set('cadrl', 'mlp_dims', '64, 32, 1')
        policy_config.set('cadrl', 'multiagent_training', 'false')
        ...
        # ========================================
        # CADRL POLICY LOADING (test4.py specific)
        # ========================================
        try:
            print(f"[INFO] Loading CADRL policy")
            ...
            print(f"[ERROR] CADRL initialization failed: {e}")
            raise
        ...
        # [FIX CADRL] Wrap onestep_lookahead to return ObservableState objects
        original_lookahead = env.onestep_lookahead
        def wrapped_lookahead(action):
            result = original_lookahead(action)
            if isinstance(result, tuple):
                if len(result) == 5:
                    # If 5 values, drop the last one (likely truncated flag from gym)
                    ob, reward, done, info, _ = result
                else:
                    ob, reward, done, info = result
                # Convert numpy array back to list for CADRL
                if isinstance(ob, np.ndarray):
                    ob = ob.tolist()
                return ob, reward, done, info
            return result
        env.onestep_lookahead = wrapped_lookahead
        ...
        # [FIX CADRL] Monkey-patch predict to handle FullState + ObservableState
        if hasattr(policy, 'predict') and policy.__class__.__name__ == 'CADRL':
            original_predict = policy.predict
            def fixed_predict(state):
                from crowd_sim.envs.utils.state import FullState
                # Call original but fix the state concatenation
                if policy.phase is None or policy.device is None:
                    raise AttributeError('Phase, device attributes have to be set!')
                if policy.phase == 'train' and policy.epsilon is None:
                    raise AttributeError('Epsilon attribute has to be set in training phase')

                if policy.reach_destination(state):
                    return ActionXY(0, 0) if policy.kinematics == 'holonomic' else ActionXY(0, 0)
                if policy.action_space is None:
                    policy.build_action_space(state.self_state.v_pref)

                probability = np.random.random()
                if policy.phase == 'train' and probability < policy.epsilon:
                    max_action = policy.action_space[np.random.choice(len(policy.action_space))]
                else:
                    policy.action_values = list()
                    max_min_value = float('-inf')
                    max_action = None
                    for action in policy.action_space:
                        next_self_state = policy.propagate(state.self_state, action)
                        ob, reward, done, info = policy.env.onestep_lookahead(action)
                        # FIX: ob is a flat list, reshape to (n_humans, 5)
                        next_self_state_tuple = (next_self_state.px, next_self_state.py, next_self_state.vx,
                                                 next_self_state.vy, next_self_state.radius, next_self_state.gx,
                                                 next_self_state.gy, next_self_state.v_pref, next_self_state.theta)
                        if isinstance(ob, list) and len(ob) > 0 and isinstance(ob[0], (int, float)):
                            # Flat list, reshape to (n_humans, 5)
                            ob = [ob[i:i+5] for i in range(0, len(ob), 5)]
                        batch_next_states = torch.cat([torch.Tensor([next_self_state_tuple + tuple(h)]).to(policy.device)
                                                      for h in ob], dim=0)
                        outputs = policy.model(policy.rotate(batch_next_states))
                        min_output, min_index = torch.min(outputs, 0)
                        min_value = reward + pow(policy.gamma, policy.time_step * state.self_state.v_pref) * min_output.data.item()
                        policy.action_values.append(min_value)
