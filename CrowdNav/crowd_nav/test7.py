...
from policy.dsrnn_policy import DSRNNPolicy  # Direct import for DSRNN
...
def main():
    ...
    parser.add_argument('--policy', type=str, default="dsrnn", help="Policy name (only dsrnn supported)")
    parser.add_argument('--model_dir', type=str, default='runs/mamba_vl', help='Model directory')
    parser.add_argument('--weights', type=str, default="27776.pt", help="Checkpoint name or \"latest\"")
    ...
    print("[INFO] ========================================")
    print("[INFO] TEST7.PY - DSRNN Baseline Testing")
    print("[INFO] ========================================")
    ...
    # [FIX DSRNN] Set default tensor type to CUDA globally for entire test
    if device.type == 'cuda':
        torch.set_default_tensor_type('torch.cuda.FloatTensor')
        print("[INFO] Set default tensor type to CUDA")
    if sarl_value_net:
    ...
    for case_idx, case in enumerate(test_cases):
        ...
        # ========================================
        # DSRNN POLICY LOADING (test7.py specific)
        # ========================================
        try:
            print(f"[INFO] Loading DSRNN policy")
            # DSRNN direct import (policy_factory may not have it registered)
            policy = DSRNNPolicy()
            ...
            print(f"[ERROR] DSRNN initialization failed: {e}")
            raise

        # [FIX CRITICAL] Ensure device and phase are set
        if hasattr(policy, 'set_device'):
            policy.set_device(device)
        # Force policy.device to match
        if hasattr(policy, 'device'):
            policy.device = device
        if hasattr(policy, 'set_phase'):
            policy.set_phase('test')

        # [FIX DSRNN] Reset episode stats to initialize rnn_hxs on correct device
        if hasattr(policy, 'reset_episode_stats'):
            policy.reset_episode_stats()
        ...
        if hasattr(policy, 'load_state_dict'):
            ...
            print("[DEBUG] Loaded weights via policy.load_state_dict()")
        elif hasattr(policy, 'model'):
            policy.model.load_state_dict(state_dict)
            print("[DEBUG] Loaded weights via policy.model.load_state_dict()")

        # [FIX DSRNN] Move model to GPU and register all tensor attributes as buffers
        if hasattr(policy, 'model') and hasattr(policy.model, 'base'):
            # First move model normally
            policy.model.to(device)

            # Then find and move all tensor attributes in base module
                ...
                if not attr_name.startswith('_'):
                    try:
                        ...
                        if torch.is_tensor(attr) and not isinstance(attr, torch.nn.Parameter):
                            # Register as buffer so it moves with .to()
                            base.register_buffer(attr_name, attr.to(device))
                    except:
                        pass

            # Move again to ensure all buffers are on GPU
            policy.model.to(device)
