...
## 截至暂停的进展总报告（2026-09-19汇总）

**结论：局部代价如何组合，已有正面证据；贝叶斯额外帮助泛化，尚未证实。**
本节把下方分散的实验记录汇总起来，不恢复训练，不把未完成实验当作结论。

### 1. 这轮究竟在做什么

目标始终是：**只在5人环境学局部交互，再组合到10/12/20人导航**。
没有转去寻找新隐藏类型，也没有改为人机让行问题。

固定边界：`robot.visible=false`；离散80动作；训练、验证和选模型只用5人；
保留IL初始化与RL阶段；不引入DAgger、PPO、GDBN或Mamba；没有生成PDF。
本轮算法修改集中在已有 `crowd_nav/bayesian.py` 和 `crowd_nav/train.py`，
没有再新增算法模块。报告写在本README，原始结果与检查点另外保留。

当前网络不是直接将整个20人局面压成一个特征再猜动作，而是：

```text
每个候选动作 + 每个行人的公开位置/速度及预测分布
    -> 共享局部网络，计算逐人代价
    -> 多人组合算子
    -> 与机器人自身的动作价值合成
    -> 选择离散动作
...
训练：ORCA示范 -> BC初始化 -> RL
RL已实际运行两条：自临界策略梯度、Double DQN
```

`prior`不是“没有不确定性”：它使用固定尺度的预测分布。
`FULL`使用公开速度增量递归更新Normal-Inverse-Gamma后验，再用五个预测点近似。
它不是旧GDBN，也不是行人真实类型；五点近似不等于完整Student-t分布积分。
因此，FULL与prior的差别才涉及递归估计是否有额外作用。

### 2. 已经做完的实验

以下各行使用不同测试批次，只能在各自行内与对应对照比较，不能跨行排名。
百分比为20人成功率，圆形与方形场合并。三种子实验每种方法为300次执行，
不是300个独立布局；同一批布局被不同种子与方法复用。

| 实验 | 实际改动或检验 | 完整结果 | 判定 |
| --- | --- | --- | --- |
| 时间/排序特征头 | 加入多时刻净空统计或前三危险者的有序特征，做IL及RL | 原IL 63.00%；统计头54.33%；排序头52.67% | 已完成，未改善 |
| 稠密局部事件监督 | 用真实行人后续轨迹作训练标签，学习逐人碰撞事件，再比较概率组合 | 对应原IL+RL 73.00%；事件prior 51.00%；FULL 45.00% | 已完成，未改善；未来仅作训练标签 |
| 调整事件训练顺序 | 事件预测训练后直接RL，不再额外BC | 对应原IL+RL 71.00%；prior 54.67%；FULL 49.67% | 已完成，未改善 |
| 五人验证选择训练顺序 | 事先规定按5人表现选择是否做额外BC，再用该分支的RL结果 | prior 54.67%；FULL 52.00% | 已完成，仍未改善 |
| 同预算组合对照 | 共享同类局部网络，分别训练max–sum组合和注意力组合 | 固定不确定性下72.00%对44.00% | 已完成，本轮最重要正结果 |
| 局部相对代价组合 | 先减去每个人跨动作的最低代价，再组合，参数量不变 | prior 73.33%对原组合72.00%；FULL 43.67%对61.67% | 已完成，不能主张改善 |

这些不只是零训练诊断。时间特征、事件监督、注意力、相对代价均已改代码并训练。
但不是每个策略梯度模型都跑满RL：部分未过IL门槛，部分因五人验证退化提前回滚。
下方各实验保留具体失败与预算差异，不能把它们统一称为“充分RL训练后的失败”。

### 3. 正结果到底是什么，能说明多远

这组对照使用相同500条五人示范回合、12000次BC更新、相同训练种子与开发集，
随后按相同策略梯度训练/回滚规则执行。模型先冻结，再测试。
注意力组是**受控注意力聚合器，不是复现SARL**。

| 选定策略 | 5人SR | 10人SR | 12人SR | 20人SR | 20人成功/碰撞/超时 |
| --- | ---: | ---: | ---: | ---: | --- |
| max–sum + 固定不确定性 | 93.67% | 89.00% | 82.67% | 72.00% | 216/45/39 |
| 注意力 + 固定不确定性 | 88.33% | 69.00% | 59.33% | 44.00% | 132/154/14 |
| max–sum + FULL | 87.00% | 75.67% | 71.33% | 61.67% | 185/65/50 |
| 注意力 + FULL | 86.00% | 71.67% | 53.33% | 32.67% | 98/184/18 |

- 固定不确定性下，三个种子的20人SR分别为 **65/66/85% 对 47/17/68%**，均是max–sum领先。
- 两组五人差5.33个百分点，20人差28个百分点。按种子和配对case重采样，
  20人差值的探索性95%区间为[10.67,48.00]个百分点；差距扩大部分为[6.00,41.00]。
- 只看RL之前的IL，20人也有 **69.67% 对40.33%**，所以差距并非全由RL门槛或回滚产生。
- 但组合算子也改变局部网络的训练梯度。这不是“同一套已训练局部权重，仅执行时换算子”的实验。
- 只有三种子，区间未作多重比较校正；不能扩大为所有注意力网络都差，也不能宣称max–sum本身是新算法。
- FULL在max–sum下反而比prior低10.33个百分点，区间[-25.00,8.00]。
  **上述28个百分点不能算作贝叶斯收益。**

局部相对代价版本的73.33%只比72.00%高1.33个百分点，区间[-10.00,13.33]，
三个种子差值为+6/+6/-8。它虽通过数学与工程自检，但没有可靠性能提升证据。

### 4. 暂停时，哪些东西还没有做完

**A. 原max–sum组合的Double DQN对照：训练完成，评估未完成。**

六组（prior/FULL各三种子）均实际完成20000环境步、19937次TD更新，
总计120000环境步、119622次TD更新。使用同一组已保存IL起点，无BC联合损失，
不因IL低于90%而禁止学习。它回答的是“换成充分更新的离散值学习是否不同”。

暂停时，完整结果JSON只保存了 **8/12个检查点评估记录**，即前两个种子的
选定版与最后版，共3200次完整记录。第三种子只有部分日志输出，不能拼成完整主表。
部分五人高分模型在高人数下大量超时；这只是已观察到的现象，完整三种子比较尚未完成。
最后版即使在20人更好，也不替换按五人验证选择的主模型，避免测试集选模。

**B. 新的“先组合、后取期望”：代码和自检完成，训练未完成，尚无测试结果。**

旧算子是 `max_i E[C_i(a)]`，新算子是 `E[max_i C_i(a)]`，求和项不变。
新实现对现有五点分布在“逐人条件独立”假设下做精确有限分布计算，参数仍为19588个。
它不是完整真实人群分布，也不是已经校准的联合碰撞概率。
CPU/CUDA自检已覆盖两人25分支穷举、梯度、排列/填充不变性和单人/确定性退化。

| 已启动训练 | IL更新数 | 五人IL成功率 | 暂停前最近保存的RL步数 | 已保存TD更新数 |
| --- | ---: | ---: | ---: | ---: |
| prior，2407 | 12000 | 96% | 16000 | 15937 |
| FULL，2407 | 12000 | 94% | 15000 | 14937 |
| prior，4807 | 12000 | 91% | 14000 | 13937 |
| FULL，4807 | 12000 | 86% | 14000 | 13937 |

7207的两组尚未启动；上表是最近持久化进度，不包含保存后被中止的零散步数。
计划使用的新测试case为10000000--10000049，**尚未打开这项测试**。
这项不能写成成功，也不能写成失败，更不能说“所有组合可能性已经测完”。

### 5. 代码、数据与暂停状态

| 内容 | 位置 |
| --- | --- |
| 算法实现 | `crowd_nav/bayesian.py`：注意力、相对代价、联合期望等组合及自检所需接口 |
| 训练与自检 | `crowd_nav/train.py`：训练入口、独立验证case、Double DQN门槛覆盖的显式记录、回归测试 |
| 已完成主对照 | `/home/abc/temp/local_event_results_20260918/frozen_attention_evaluation.json`，24条检查点记录，其中7条复用相同IL权重；实际6800次执行 |
| 相对代价实验 | 同目录 `frozen_regret_evaluation.json`，12条记录，其中2条复用；实际4000次执行 |
| 时间特征实验 | `/home/abc/temp/local_ordered_20260918/` |
| 事件监督与顺序实验 | 主数据目录内 `local_event_belief_20260918/`、`local_event_order_20260918/` |
| 已完成模型 | 主数据目录内 `matched_attention_runs/`、`matched_regret_runs/`、`matched_ddqn_runs/` |
| 暂停快照 | 主数据目录内 `local_joint_matched_20260918/` 与 `local_ddqn_matched_20260918/`，包括 `user_stop.json` |

Git分支为 `fix-junction-candidates`。关键记录：`3b82805`为组合对照结果，
`efb96ee`为相对代价阴性结果，`1b8402c`为联合期望实现与事前方案，
`c7be556`为暂停声明。本次只补汇总报告，不重启任何任务。
模型、原始回合记录保存在上述本地目录；不声称这些大体积文件全部已入Git。

暂停时已停止本轮训练与评估，未停止无关ECG任务。现有检查点保存了模型，
**没有完整优化器、replay和随机数快照，不能承诺逐位续训**。

### 6. 执行复盘与交付边界

我确实做了代码改造、训练和冻结测试，不是只做便宜诊断；也拿到了组合方式的正证据。
但我连续展开了过多后续变体，报告只按实验零散追加，没有及时给出统一进度和成本清单。
这是执行与沟通问题，不应让用户靠追问才能知道做到了哪里。
第一个注意力试跑还误用了4000步margin配置，发现后在高人数测试前作废，
所有臂统一重跑12000步softmax配置；原试跑与作废原因保留，未拿来作正式结论。

**现在可交付的是：可运行代码、一个三种子受控组合对照的正结果、多项完整阴性结果，
以及清楚标记的未完成实验。不是“贝叶斯泛化方法已经成功”，也不是“论文已可发表”。**
若以后恢复，先由用户决定是否完成未完成项，不自动再展开新分支。
